...
const UploadBookModal = ({ onUpload, onUploadPdf, onClose, uploading, selectedSchool, selectedClass, selectedSubject, schools, classes, subjects, book = null }) => {
 // Get helper data
 ...
 const isEditMode = !!book;
  ...
  const handleFullBookPdfChange = (e) => {
  ...
  const handleFullBookPdfUpload = async () => {
    if (!fullBookPdf || !book?.id) return;
    setUploadingPdf(true);
    await onUploadPdf(book.id, fullBookPdf);
    setFullBookPdf(null);
...
>
  <FaTimes className="text-xl" />
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          {/* Full Book PDF Upload — Edit mode only */}
          {isEditMode && (
            <div className="border-t-2 border-gray-200 dark:border-gray-700 pt-6">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-3 flex items-center">
                <FaFilePdf className="mr-2 text-red-500" /> Upload Full Book PDF
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                Optionally upload a single PDF of the entire book. It will be saved to the book's FTP folder and update the book's <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">pdf_url</code>.
              </p>
              <div className="flex items-center gap-3">
                <input
                  type="file"
                  accept=".pdf"
                  onChange={handleFullBookPdfChange}
                  disabled={uploading || uploadingPdf}
                  className="flex-1 px-3 py-2 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-red-50 dark:file:bg-red-900/30 file:text-red-700 dark:file:text-red-300 disabled:opacity-50"
                />
                <button
                  type="button"
                  onClick={handleFullBookPdfUpload}
                  disabled={!fullBookPdf || uploading || uploadingPdf}
                  className="px-4 py-2 bg-red-600 dark:bg-red-500 text-white font-semibold rounded-lg hover:bg-red-700 dark:hover:bg-red-600 transition-colors flex items-center disabled:opacity-50 whitespace-nowrap"
                >
                  {uploadingPdf ? (
                    <>
              ...
              {fullBookPdf && (
                <p className="mt-2 text-xs text-green-600 dark:text-green-400 flex items-center">
                  ...
                  {fullBookPdf.name} ({(fullBookPdf.size / 1024 / 1024).toFixed(2)} MB)
                </p>
              )}
              {book?.pdf_url && (
                <p className="mt-2 text-xs text-blue-600 dark:text-blue-400">
                  ✓ Current PDF: <a href={book.pdf_url} target="_blank" rel="noreferrer" className="underline hover:text-blue-800">View existing</a>
                </p>
              )}
            </div>
          )}
