import { FaTrash, FaBookOpen, FaTimes } from "react-icons/fa";

const DeleteSubjectModal = ({ subject, onDelete, onClose }) => {
  return (
    <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">Delete Subject</h2>
          <p className="text-gray-600 dark:text-gray-300">
            Are you sure you want to delete{" "}
            <span className="font-bold text-gray-800 dark:text-white">{subject?.name}</span>?
        ...
        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 mb-6">
          <div className="flex items-center">
            ...
            <div>
              <p className="font-bold text-gray-800 dark:text-white">{subject?.name}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {subject?.code ? `Code: ${subject.code} • ` : ''}
                {subject?.class_name} • {subject?.school_name}
              </p>
            </div>
          </div>
        </div>
